...
rects = [[] for i in range(4444)]
color_pattern = ['1','0', '0.7', '0.4','0.6','0.9', '0.2', '0.5', '0.8', '0.1', '0.4','0.7', '1'] #light and contrasting colors
hatch_pattern = ['\\','|','/','x','+','/', ' ', 'x', ' ', '-'] 

datasum2 = map(add, data[0], data[1])
name = 'rects' + str(j)
...
rects[3] = ax.bar(ind + width * 2, data[3], width, color=color_pattern[3], hatch = hatch_pattern[3 % len(hatch_pattern)], bottom=map(add, datasum2, data[2]))
...
ax.legend((rects[0][0], rects[1][0], rects[2][0], rects[3][0]), types, loc = "upper right", prop = {'size': 35}, ncol = 2)
plt.show()
